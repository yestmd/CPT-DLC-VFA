...
savepath <- file.path(diskpath, "/CPT_TOT_manuscript/data processing/fig/15_amph_ori_index_LH_V2")
...
output_file <- file("amph orientation index event mixed_model_report only interaction.txt", "w")
...
for (df_name in df_list) {
  ...
  dteff <- lmer(ori_index ~ min_15bin*dosage + (1|id_cohort), data=dt) 
  print(anova(dteff)) 
  ...
  diskpath <- "C:/Users/ye.li/OneDrive - Lieber Institute for Brain Development/08_25_2022"
  savepath <- file.path(diskpath, "/CPT_TOT_manuscript/data processing/fig/15_amph_ori_index_LH_V2")
